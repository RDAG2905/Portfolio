CREATE PROCEDURE [GetAgendaPorFechasNotificacionExcel]
    @fechaDesde [nvarchar](max),
    @fechaHasta [nvarchar](max)
AS
BEGIN
     	SELECT tev.fecha,tev.EventoId,td.DenunciaId,
    tt.Agendable as Agendable,tt.Nombre as nombreTipoEvento,
    tor.Nombre as Organismo,(tde.apellido + ' ' + tde.nombre) as Denunciante,
    tde.NroDocumento as dniDenunciante,td.EXPEDIENTE_ID as Nro_Expediente,
    td.OBJETORECLAMO as Nro_Linea ,tser.Nombre as Servicio,tpro.Nombre as Provincia,
    td.TRAMITECRM as TramiteCRM,tsub.Nombre as Estado_Actual,treq.Nombre as Requerimiento_Informe,
    	                    tso.Nombre as Solucionado,tev.CONTESTADO as Contestado,tes.Nombre as Estudio,
    							tev.tipoEvento,td.ESTUDIO_ID,tor.Provincia_Id,
    							tev.REQUERIMIENTOINFORME,td.ORGANISMO_ID,tev.SOLUCIONADO as SolucionadoId,
    							tev.ResIntId,td.DENUNCIANTE_ID
    from tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    
    inner join tDenunciantes tde
    on td.DENUNCIANTE_ID = tde.DenuncianteId
    left join tEstudios tes
    on td.ESTUDIO_ID = tes.Id
    	                    inner join tServicios tser
    	                    on td.SERV_DEN_ID = tser.Id
    	                    left join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    inner join tSubEstados tsub
    	                    on td.CONCILIACION_ID = tsub.Id
    inner join teventos tev
    on td.DenunciaId = tev.DenunciaId
    inner join tTipoEvento tt
    on tev.tipoEvento = tt.Id
    	                left join tSolucionado tso
    	                on tev.SOLUCIONADO = tso.Id
    	                left join tReqInforme treq
    	                on tev.REQUERIMIENTOINFORME = treq.Id              
    where td.FSELLOCIA  >= @fechaDesde
    	             and td.FSELLOCIA <= @fechaHasta             
    
END
GO

CREATE PROCEDURE [GetAgendaPorFechasNotificacionGciaExcel]
    @fechaDesde [nvarchar](max),
    @fechaHasta [nvarchar](max)
AS
BEGIN
     	SELECT tev.fecha,tev.EventoId,td.DenunciaId,
    tt.Agendable as Agendable,tt.Nombre as nombreTipoEvento,
    tor.Nombre as Organismo,(tde.apellido + ' ' + tde.nombre) as Denunciante,
    tde.NroDocumento as dniDenunciante,td.EXPEDIENTE_ID as Nro_Expediente,
    td.OBJETORECLAMO as Nro_Linea ,tser.Nombre as Servicio,tpro.Nombre as Provincia,
    td.TRAMITECRM as TramiteCRM,tsub.Nombre as Estado_Actual,treq.Nombre as Requerimiento_Informe,
    	                    tso.Nombre as Solucionado,tev.CONTESTADO as Contestado,tes.Nombre as Estudio,
    							tev.tipoEvento,td.ESTUDIO_ID,tor.Provincia_Id,
    							tev.REQUERIMIENTOINFORME,td.ORGANISMO_ID,tev.SOLUCIONADO as SolucionadoId,
    							tev.ResIntId,td.DENUNCIANTE_ID
    from tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    
    inner join tDenunciantes tde
    on td.DENUNCIANTE_ID = tde.DenuncianteId
    left join tEstudios tes
    on td.ESTUDIO_ID = tes.Id
    	                    inner join tServicios tser
    	                    on td.SERV_DEN_ID = tser.Id
    	                    left join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    inner join tSubEstados tsub
    	                    on td.CONCILIACION_ID = tsub.Id
    inner join teventos tev
    on td.DenunciaId = tev.DenunciaId
    inner join tTipoEvento tt
    on tev.tipoEvento = tt.Id
    	                left join tSolucionado tso
    	                on tev.SOLUCIONADO = tso.Id
    	                left join tReqInforme treq
    	                on tev.REQUERIMIENTOINFORME = treq.Id              
    where td.FSELLOGCIADC  >= @fechaDesde
    	             and td.FSELLOGCIADC <= @fechaHasta  
END
GO

CREATE PROCEDURE [GetEventosPorFechasNotificacion]
    @fechaDesde [nvarchar](max),
    @fechaHasta [nvarchar](max)
AS
BEGIN
     	SELECT tev.fecha,tev.EventoId,td.DenunciaId,tt.Id as tipoEventoId,tt.Agendable as Agendable,tt.Nombre as tipoEvento,tor.Id as OrganismoId,tor.Nombre as Organismo,(tde.apellido + ' ' + tde.nombre) as Denunciante,tde.NroDocumento as dniDenunciante,tor.Provincia_Id as ProvinciaId,
    td.TRAMITECRM as TramiteCRM,tev.REQUERIMIENTOINFORME as ReqInformeId,treq.Nombre as RequerimientoInforme,tev.SOLUCIONADO as SolucionadoId,tso.Nombre as Solucionado,tev.CONTESTADO as Contestado,td.RESP_INT_ID,tes.Id as estudioId,tes.Nombre as Estudio
    from tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tDenunciantes tde
    on td.DENUNCIANTE_ID = tde.DenuncianteId
    inner join tEstudios tes
    on td.ESTUDIO_ID = tes.Id
    inner join teventos tev
    on td.DenunciaId = tev.DenunciaId
    inner join tTipoEvento tt
    on tev.tipoEvento = tt.Id
    	                left join tSolucionado tso
    	                on tev.SOLUCIONADO = tso.Id
    	                left join tReqInforme treq
    	                on tev.REQUERIMIENTOINFORME = treq.Id
    						 where td.FSELLOCIA  >= @fechaDesde
    	                 and td.FSELLOCIA <= @fechaHasta              
    
END
GO

CREATE PROCEDURE [GetEventosPorFechasNotificacionGcia]
    @fechaDesde [nvarchar](max),
    @fechaHasta [nvarchar](max)
AS
BEGIN
     SELECT tev.fecha,tev.EventoId,td.DenunciaId,tt.Id as tipoEventoId,tt.Agendable as Agendable,tt.Nombre as tipoEvento,tor.Id as OrganismoId,tor.Nombre as Organismo,(tde.apellido + ' ' + tde.nombre) as Denunciante,tde.NroDocumento as dniDenunciante,tor.Provincia_Id as ProvinciaId,
    td.TRAMITECRM as TramiteCRM,tev.REQUERIMIENTOINFORME as ReqInformeId,treq.Nombre as RequerimientoInforme,tev.SOLUCIONADO as SolucionadoId,tso.Nombre as Solucionado,tev.CONTESTADO as Contestado,td.RESP_INT_ID,tes.Id as estudioId,tes.Nombre as Estudio
    from tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tDenunciantes tde
    on td.DENUNCIANTE_ID = tde.DenuncianteId
    inner join tEstudios tes
    on td.ESTUDIO_ID = tes.Id
    inner join teventos tev
    on td.DenunciaId = tev.DenunciaId
    inner join tTipoEvento tt
    on tev.tipoEvento = tt.Id
    	                left join tSolucionado tso
    	                on tev.SOLUCIONADO = tso.Id
    	                left join tReqInforme treq
    	                on tev.REQUERIMIENTOINFORME = treq.Id
    						 where td.FSELLOGCIADC  >= @fechaDesde
    	                 and td.FSELLOGCIADC <= @fechaHasta              
    
END
INSERT [dbo].[__MigrationHistory]([MigrationId], [ContextKey], [Model], [ProductVersion])
VALUES (N'201911241420576_createSPGetAgendaPorFechaNotificacionEXCEL', N'Greco2.Models.NuevoDbContext',   , N'6.2.0-61023')

