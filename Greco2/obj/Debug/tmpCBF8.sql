CREATE PROCEDURE [ExcelGetDenunciasPorOrganismo]
    @organismoId [int]
AS
BEGIN
     	
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    						
    						
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    	                and tor.Id = @organismoId
    left join tLocalidades tlo
    	                on tor.Localidad_Id = tlo.Id
    	                left join tProvincias tpro
    	                on tor.Provincia_Id = tpro.Id
    	                left join tRegiones treg
    	                on tor.Region_Id = treg.Id
    	                inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id	
    	                inner join tEstados test
    	                on td.ETAPA_ID = test.Id
    	                left join tModalidadGestion tmod
    	                on td.MODALIDADGESTION = tmod.Id
    	                left join tSubTipoProceso tsubtp
    	                on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                left join tMotivoDeReclamo tmotr
    	                on td.RECLAMO_ID = tmotr.Id
    
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorEtapaProcesal,]
    @etapaProcesalId [int]
AS
BEGIN
     	
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    left join tLocalidades tlo
    	                on tor.Localidad_Id = tlo.Id
    	                left join tProvincias tpro
    	                on tor.Provincia_Id = tpro.Id
    	                left join tRegiones treg
    	                on tor.Region_Id = treg.Id
    
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id
    	                
    left join tEstados test
    	                on td.ETAPA_ID = test.Id
    	                left join tModalidadGestion tmod
    	                on td.MODALIDADGESTION = tmod.Id
    	                left join tSubTipoProceso tsubtp
    	                on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                left join tMotivoDeReclamo tmotr
    	                on td.RECLAMO_ID = tmotr.Id
    where td.ETAPA_ID = @etapaProcesalId
    
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorRegion]
    @regionId [int]
AS
BEGIN
     	
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    	                    and tor.Region_Id = @regionId
    left join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    left join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorProvincia]
    @provinciaId [int]
AS
BEGIN
     	
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    left join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    						and tpro.id = @provinciaId
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorLocalidad]
    @localidadId [int]
AS
BEGIN
     	
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    						and tlo.Id = @localidadId
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    
    
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorServicio]
    @servicioId [int]
AS
BEGIN
     	 
    
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						where td.SERV_DEN_ID = @servicioId
    
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorIdDenuncia]
    @denunciaId [int]
AS
BEGIN
     	
    
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						where td.DenunciaId = @denunciaId                                               
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorEstudio]
    @estudioId [int]
AS
BEGIN
     	
    
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						where td.ESTUDIO_ID = @estudioId                                               
    
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorApellido]
    @apellido [nvarchar](max)
AS
BEGIN
     	  
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    and tden.apellido like '' + @apellido + '%'
    						inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorDni]
    @dni [int]
AS
BEGIN
     	                  
    
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    and tden.NroDocumento = @dni
    						inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						
    
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorEstadoConciliacion]
    @estadoId [int]
AS
BEGIN
     	
    
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    
    						inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						where td.CONCILIACION_ID = @estadoId
    
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorNroLinea]
    @nroLinea [nvarchar](max)
AS
BEGIN
     	
    
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    
    						inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						where td.OBJETORECLAMO = @nroLinea                                               
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorMotivoDeReclamo]
    @reclamoId [int]
AS
BEGIN
     
    
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId                        
    						inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						where td.RECLAMO_ID = @reclamoId 
    
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorNroExpediente]
    @nroExpediente [nvarchar](max)
AS
BEGIN
     	                     
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    
    						inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						where td.EXPEDIENTE_ID = @nroExpediente                                                              
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorResponsable]
    @responsableId [int]
AS
BEGIN
     
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    
    						inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						where td.RESP_INT_ID = @responsableId
    
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorFechasDeNotificacion]
    @fechaDesde [nvarchar](max),
    @fechaHasta [nvarchar](max)
AS
BEGIN
     	
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId                       
    						inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						where td.FSELLOCIA >= @fechaDesde
    						and td.FSELLOCIA <= @fechaHasta                                             
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorFechaDeNotificacionDesde]
    @fechaDesde [nvarchar](max)
AS
BEGIN
     	
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId                       
    						inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						where td.FSELLOCIA >= @fechaDesde
    						                                                
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorFechaDeNotificacionHasta]
    @fechaHasta [nvarchar](max)
AS
BEGIN
    
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId                       
    						inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						where td.FSELLOCIA <= @fechaHasta
    				
    
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorFechasDeNotificacionGcia]
    @fechaDesde [nvarchar](max),
    @fechaHasta [nvarchar](max)
AS
BEGIN
     	
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    
    						inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						where td.FSELLOGCIADC >= @fechaDesde
    						and td.FSELLOGCIADC <= @fechaHasta
    
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorFechaDeNotificacionGciaDesde]
    @fechaDesde [nvarchar](max)
AS
BEGIN
     	
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    
    						inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						where td.FSELLOGCIADC >= @fechaDesde
    				                               
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorFechaDeNotificacionGciaHasta]
    @fechaHasta [nvarchar](max)
AS
BEGIN
     	
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    
    						inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						where td.FSELLOGCIADC <= @fechaHasta
    				                               
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorNombreDenunciante]
    @nombre [nvarchar](max)
AS
BEGIN
     	
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    and tden.nombre like ''+ @nombre +'%'
    						inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    
    	                                  
    
END
GO

CREATE PROCEDURE [ExcelGetDenunciasPorTramiteCRM]
    @tramiteCRM [nvarchar](max)
AS
BEGIN
     	
    
    Select td.DenunciaId,td.EXPEDIENTE_ID,td.FCREACION,tor.Nombre as Organismo,ts.Nombre as Servicio,
    testu.Nombre as Estudio,tden.apellido as Apellido,tden.nombre as Nombre,tsub.Nombre as EstadoActual ,ttp.Nombre TipoProceso
    ,td.FSELLOCIA,td.FSELLOGCIADC,
    	                td.ETAPA_ID,td.ORGANISMO_ID,tor.Region_Id,
    	                tor.Provincia_Id,tor.Localidad_Id,
    	                td.SERV_DEN_ID,td.ESTUDIO_ID,
    	                td.TIPO_PRO_ID,tden.apellido,tden.nombre,tden.NroDocumento,
    	                td.CONCILIACION_ID,td.RECLAMO_ID,td.TRAMITECRM,
    	                td.RESP_INT_ID,td.PARENTDENUNCIAID,
    						test.Nombre as Etapa_Procesal,
    						td.CREATIONPERSON as Usuario_Creador,
    						td.OBJETORECLAMO as nro_Linea,
    						tlo.Nombre as Localidad,
    						tpro.Nombre as Provincia,
    						treg.Nombre as Region,
    						tmod.Nombre as Modalidad_Gestion,
    						tsubtp.Nombre as Sub_Tipo_Proceso,
    						tmotr.Nombre as Motivo_Reclamo,
    						td.FECHARESULTADO
    from  tDenuncias td
    inner join tOrganismos tor
    on td.ORGANISMO_ID = tor.Id
    inner join tLocalidades tlo
    	                    on tor.Localidad_Id = tlo.Id
    	                    inner join tProvincias tpro
    	                    on tor.Provincia_Id = tpro.Id
    	                    left join tRegiones treg
    	                    on tor.Region_Id = treg.Id
    inner join tServicios ts
    on td.SERV_DEN_ID = ts.Id 
    left join tEstudios testu
    on td.ESTUDIO_ID = testu.Id
    inner join tDenunciantes tden
    on td.DENUNCIANTE_ID = tden.DenuncianteId
    
    						inner join tSubEstados tsub
    on td.CONCILIACION_ID = tsub.Id
    inner join tTipoProceso ttp
    on td.TIPO_PRO_ID = ttp.Id   
    left join tEstados test
    	                    on td.ETAPA_ID = test.Id
    	                    left join tModalidadGestion tmod
    	                    on td.MODALIDADGESTION = tmod.Id
    	                    left join tSubTipoProceso tsubtp
    	                    on td.SUBTIPO_PRO_ID = tsubtp.Id
    	                    left join tMotivoDeReclamo tmotr
    	                    on td.RECLAMO_ID = tmotr.Id
    						where td.TRAMITECRM = @tramiteCRM
    						                 
    
END
INSERT [dbo].[__MigrationHistory]([MigrationId], [ContextKey], [Model], [ProductVersion])
VALUES (N'201911231707265_SPGetDenunciasExcelPorEntidad', N'Greco2.Models.NuevoDbContext',   , N'6.2.0-61023')

